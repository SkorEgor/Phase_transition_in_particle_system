...
class Drawer:
    # ПАРАМЕТРЫ ГРАФИКОВ
    # График №1 Данные
    title_data = "График №1. Расположение частиц"
    horizontal_axis_name_data = "x [нм]"
    vertical_axis_name_data = "y [нм]"
    ...
    def particle_field(
        ...
        # Точки
        graph.axis.scatter(data_signals.particles.x, data_signals.particles.y)
        # Граница расчетной зоны
        graph.axis.plot([data_signals.start_of_border_x, data_signals.start_of_border_x,
                         data_signals.end_of_border_x, data_signals.end_of_border_x,
                         data_signals.start_of_border_x],
                 [data_signals.start_of_border_y, data_signals.end_of_border_y,
                  data_signals.end_of_border_y, data_signals.start_of_border_y,
                  data_signals.start_of_border_y],
                 color='red')
        # Векторное поле скоростей
        # graph.axis.quiver(data_signals.particles.x, data_signals.particles.y,
        #            data_signals.particles.Vx, data_signals.particles.Vy, color='b', units='xy', scale=500)
